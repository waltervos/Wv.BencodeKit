class BencodedData {
    ...
    BencodedData([String] $String) {
        $Chars = $String.ToCharArray()
        $RawBytes = ([System.Text.Encoding]::Unicode).GetBytes($Chars)
        $this.BencodedData($RawBytes)
    }

    BencodedData([System.IO.FileInfo] $File) {
        $this.BencodedData($File, [System.Text.Encoding]::Default)
    }
    
    BencodedData([System.IO.FileInfo] $File, [System.Text.Encoding] $Encoding) {
        $Stream = [System.IO.FileStream]::new([string] $File.FullName, 'Open')
        $Reader = [System.IO.BinaryReader]::new($Stream, $Encoding)
        $RawBytes = $Reader.ReadBytes($Stream.Length)
        $this.BencodedData($RawBytes)
    }

    BencodedData([byte[]] $RawBytes) {
        $this.Bytes = $RawBytes | Where-Object { $_.CompareTo([byte] 0) -ne 0 }
        # Now decode these bytes!
        # The first byte must be ascii
        # Every first byte of any bencoded item must be ascii
    }
